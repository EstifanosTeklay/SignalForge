...
send_email()           accepts on_success / on_failure callbacks.
                       Accepts an optional booking_link kwarg — when provided,
                       the Cal.com discovery-call link is appended to the HTML body
                       so the prospect can self-book without waiting for a reply.
...
Cal.com integration
-------------------
Booking links are generated by calendar_handler.get_available_slots() and
passed here as booking_link. The CALCOM_BOOKING_URL env var is used as a
fallback generic link when no prospect-specific slot is pre-generated.
"""
...
_CALCOM_BOOKING_URL = os.getenv("CALCOM_BOOKING_URL", "")
...
def send_email(
    ...
    *,
    ...
    Args:
        ...
        booking_link: Optional Cal.com discovery-call booking URL.
                      When provided, a CTA block is appended to html_body:
                        "Book a 15-min call: <booking_link>"
                      Falls back to CALCOM_BOOKING_URL env var when this is
                      None and the env var is set, so every outbound email
                      can carry a self-booking option without extra pipeline work.
    ...
    # ── Cal.com booking link injection ────────────────────────────────────────
    effective_link = booking_link or _CALCOM_BOOKING_URL or None
    if effective_link:
        cta_block = (
            f'<p style="margin-top:24px;font-size:13px;color:#555;">'
            f'<strong>Book a 15-min discovery call:</strong> '
            f'<a href="{effective_link}">{effective_link}</a>'
            f"</p>"
        )
        html_body = html_body + cta_block
