...
Cal.com integration
-------------------
send_sms() accepts an optional booking_link kwarg.  When provided, the
Cal.com discovery-call URL is appended to the SMS body as a short CTA:
  "Book your call: <booking_link>"
This makes Cal.com booking reachable from the SMS code path directly,
without requiring the prospect to return to email.
Falls back to CALCOM_BOOKING_URL env var when booking_link arg is None.
"""
...
def send_sms(
    ...
    *,
    ...
    Args:
        ...
        booking_link: Optional Cal.com discovery-call URL.  When provided (or
                      when CALCOM_BOOKING_URL env var is set), a short CTA is
                      appended to the message body:
                        "Book your call: <booking_link>"
                      The link is generated by calendar_handler.get_available_slots()
                      and passed from ConversationAgent when the prospect is ready
                      to schedule.  This keeps the booking path reachable from SMS
                      without requiring the prospect to return to email.
    ...
    if effective_link:
        cta = f" Book your call: {effective_link}"
        # Respect the 160-char single-segment limit; truncate message body if needed
        max_body_len = 160 - len(cta)
        if len(message) > max_body_len:
            message = message[:max_body_len - 3] + "..."
        message = message + cta
